pipeline {
    ...
    environment {
        ...
        CONTAINER_NAME = 'belloga-firebase-service'

        SWAGGER_ID = 'belloga-swagger'
        OPEN_API_SPEC_NAME = 'open-api-3-firebase-service.json'

        AWS_CREDENTIAL_NAME = 'belloga-aws'
        ECR_PATH = '023778162658.dkr.ecr.ap-northeast-2.amazonaws.com'
        IMAGE_NAME = '023778162658.dkr.ecr.ap-northeast-2.amazonaws.com/belloga-firebase-service'
        ...
        stage('create application-prod') {
            steps {
                dir('src/main/resources') {
                    withAWSParameterStore(credentialsId: "${env.AWS_CREDENTIAL_NAME}",
                                path: "/belloga/firebase/application/profile",
                      			naming: 'basename',
                      			regionName: 'ap-northeast-2') {
        ...
        stage('kubectl apply') {
            steps {
                ...
                sh """
                    ...
                    kubectl set image deploy firebase-service-v1 firebase-service=${IMAGE_NAME}:${BUILD_NUMBER}
                    '
                """
                }
            }
